...
Present observations one at a time with a progress indicator (e.g., `[2/6]`). For each observation, give the initial recommendation and ask the user if they want to deep-dive.
...
After walking through all observations, if the user selected any for deep-dive, ask permission before spawning subagents. Spawn confirmed selections in parallel.
